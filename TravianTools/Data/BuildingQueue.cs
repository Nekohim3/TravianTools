...
namespace TravianTools.Data
{
    ...
    {
        public List<(int idq, int type, int loc)> Queue;
        public List<(int id, int freeCount)>      FreeSlots;
 
        public BuildingQueue(dynamic d, long time)
        {
            Queue = new List<(int idq, int type, int loc)>();
            if (d.queues["1"].Count != 0)
            {
                Queue.Add((1, d.queues["1"][0].buildingType, d.queues["1"][0].locationId));
            }
            if (d.queues["2"].Count != 0)
            {
                Queue.Add((2, d.queues["2"][0].buildingType, d.queues["2"][0].locationId));
            }
            if (d.queues["4"].Count != 0)
            {
                foreach (var x in d.queues["4"])
                    Queue.Add((4, x.buildingType, x.locationId));
            }
            if (d.queues["5"].Count != 0)
            {
                Queue.Add((5, d.queues["5"][0].buildingType, d.queues["5"][0].locationId));
            }
